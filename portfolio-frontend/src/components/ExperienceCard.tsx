  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-4 gap-4">
        ...
        {/* ✅ أزرار الروابط (Link + Download) */}
        <div className="flex items-center gap-3">
          {data.appDownloadUrl && (
            <a
              href={data.appDownloadUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-blue-500/40 text-blue-300 hover:text-blue-200 hover:bg-blue-500/10 transition-colors text-sm"
              title="Download App"
            >
              <FaDownload className="w-4 h-4" />
              <span>{data.appDownloadLabel || "Download"}</span>
            </a>
          )}
